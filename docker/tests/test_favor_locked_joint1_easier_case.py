...
workspace = workspace_cls(cfg, output_dir="/workspace/results/_scratch_j1_easy")
...
N_TEST = 10  # a bit more than the earlier n=5 pilots, still cheap

fault_spec_locked_j1 = {
    'joint_idx': 0, 'q_lock': 0.0, 'fault_type': 'locked',
    'q_lo': q_lo_full, 'q_hi': q_hi_full,
}
projector = ProjectWaypoints(K=64, iters=5)
runner = FaultRobomimicImageRunner(
    output_dir="/workspace/results/_j1_locked_favor",
    dataset_path=DATASET, shape_meta=cfg.task.shape_meta,
    fault_joint_name="robot0_joint1", fault_type="locked", fault_severity=None,
    n_train=0, n_test=N_TEST, n_envs=N_TEST,
...
favor = FavorHybridImagePolicy(base_policy, fault_spec=fault_spec_locked_j1, projector=projector,
                                env_ref=runner.env, blend_floor=0.15)
log = runner.run(favor)
print(f"FAVOR (blend_floor=0.15), Lift, joint1 locked, n={N_TEST} -> test/mean_score =", log.get("test/mean_score"))
print("(B1 reference from Phase 3 pilot, same condition, n=5: 0.8)")
