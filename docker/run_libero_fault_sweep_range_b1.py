"""
B1 baseline sweep for range_reduced faults, mirroring
run_libero_fault_sweep_phase1.py exactly except for the grid source
(sweep_grid_libero_range.py) and output dir. Saves per-episode success
alongside the aggregate score (same as the locked-fault scripts after
their per-episode logging was added) so a real McNemar test is possible
against select/random_n once those are run.

Usage: python run_libero_fault_sweep_range_b1.py <task_name>
"""
import sys, os, json
...
OUT_DIR = "/workspace/results/libero_fault_sweep_range_b1"
...
workspace = workspace_cls(cfg, output_dir=f"/workspace/results/_sweep_scratch_range_b1_{task_name}")
...
for joint_name in JOINTS:
    for fault_type, severity in FAULT_CONDITIONS:
        ...
        runner = FaultRobomimicImageRunner(
            output_dir=f"/workspace/results/_sweep_run_range_b1_{task_name}",
...
print(f"TASK {task_name} RANGE B1 SWEEP COMPLETE")
