"""
Random-N (N=32, chunk_size=8) sweep for range_reduced faults, mirroring
run_libero_fault_sweep_phase2_random_n.py exactly except for grid source
and output dir. Same seeds/N/chunking as the select run above, so select
vs random_n stays a valid paired comparison for range_reduced too.

Usage: python run_libero_fault_sweep_range_random_n.py <task_name>
"""
import sys, os, json
...
OUT_DIR = "/workspace/results/libero_fault_sweep_range_random_n"
...
workspace = workspace_cls(cfg, output_dir=f"/workspace/results/_sweep_scratch_range_rn_{task_name}")
...
for joint_name in JOINTS:
    for fault_type, severity in FAULT_CONDITIONS:
        ...
        runner = FaultRobomimicImageRunner(
            output_dir=f"/workspace/results/_sweep_run_range_rn_{task_name}",
...
print(f"TASK {task_name} RANGE RANDOM_N SWEEP COMPLETE")
